...
* scp $HOME/openrc.sh ubuntu@publicip:
* ssh ubuntu@publicip
...
It is cheaper than Enter Cloud Suite but does not provide volumes (as
of June 2015) and is therefore unfit to run teuthology tests that
...
* Setup OpenStack (see above)
* apt-get install -y git python-virtualenv
* git clone -b wip-6502-openstack http://github.com/dachary/teuthology
* cp openrc.sh teuthology/teuthology/test/integration
* cd teuthology
* ./bootstrap install
* . virtualenv/bin/activate
* teuthology/test/integration/setup-openstack.sh \
  --openstack CLOUD \
  --key-file $HOME/.ssh/id_rsa \
  --setup-all
  where CLOUD must be one of ovh, entercloudsuite, redhat or suse
...
Run the suite that is used for the integration tests:
